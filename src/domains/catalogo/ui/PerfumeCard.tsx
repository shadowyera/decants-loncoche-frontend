...
export function PerfumeCard({ producto }: PerfumeCardProps) {
  ...
  // 🎨 badges más planos (sin blur ni sombras)
  const badgeBase =
      ...
      >
        ...
        <div className="relative aspect-[4/5] overflow-hidden bg-gradient-to-b from-neutral-900 to-black">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
            ...
            {isOutOfStock && (
              <span className={`${badgeBase} bg-zinc-600 text-white border-white/10`}>
                Sin stock
              </span>
            )}

            {!isOutOfStock && isLowStock && (
              <span className={`${badgeBase} bg-red-500 text-white border-white/10`}>
                {stock !== undefined
                  ? stock === 1
                    ? "Queda 1 unidad"
            ...
            {producto.nuevo && (
              <span className={`${badgeBase} bg-accent text-white border-white/10`}>
                Nuevo
              </span>
            )}

            {producto.masVendido && (
              <span className={`${badgeBase} bg-yellow-400 text-black border-white/10`}>
                🔥 Top
              </span>
            )}
